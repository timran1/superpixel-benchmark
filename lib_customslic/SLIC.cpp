...
//===========================================================================
void SLIC::DoRGBtoLABConversion(const cv::Mat &mat, cv::Mat &out, int padding_c_left, int padding_r_up)
{
...
{
	DefineImagePixelsAssociation();

	if (args.perturbseeds)
	{
		vector<float> edges;
		DetectLabEdges (edges);
		PerturbSeeds (edges);
	}
...
inline void SLIC::associate_cluster_to_pixel(int vect_index, int pixel_index, int row_start, int row_length, int cluster_num)
{
...
void SLIC::DefineImagePixelsAssociation()
{
...
float SLIC::calc_dist (const Pixel& p1, const Pixel& p2, float invwt)
{
	Pixel diff = p2 - p1;
...
void SLIC::PerformSuperpixelSLICIterationPPA ()
{
	...
	for( int k = 0; k < numk; k++ )
	{
		...
		float inv = 1/float (clustersize[k]);//computing inverse now to multiply, than divide later
